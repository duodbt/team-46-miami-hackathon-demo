...
with st.sidebar:
    ...
    choice = st.sidebar.selectbox(
        "Source", choices, disabled=True, help="Upload disabled for demo version")
...
mock_extracted_text = '1 "Robot" comes from the Czech word robota, meaning"drudgery;" and first appeared in the 1921 play RUR (Rossum\'s Universal Robots): The drama ends badly when the machines rise up and kill their creators, leaving a sole lonely survivor: 2 They say it was an accident: The first known case of robot homicide occurred in 1981, when a robotic arm crushed a Japanese Kawasaki factory worker: 3 More than a million industrial robots are now in use, nearly half of them in Japan: 4 Archytas of Tarentum; a of Plato\'s, built a mechanical bird driven by a jet of steam or compressed air_-arguably history\'s first robot ~in the fifth century BC 5 Leonardo da Vinci drew up plans for an armored humanoid machine in 1495. Engineer Mark Rosheim has created a functionalminiature version for NASA to help colonize Mars: 6 Slow but steady: The real Mars robots, Spirit andOpportunity, have logged 10.5 miles trudging across the Red Planet formore than three years The unstoppable droids were built to last 90days 7 The United States\' military corps of 4000 robots includes reconnaissance Talon bots that scout for roadside bombs in Iraq and PackBots that poked around for Osama bin Laden\'s hideout in Afghanistan. Apparently without much success'
# =================================================== MOCK DATA END
...
with tab1:
    if choice == "Image":
        uploaded_file = st.sidebar.file_uploader(
            "", type=['png'], disabled=True, help="Upload disabled for demo version")

        if not st.session_state.new_source_summary and not st.session_state.new_source_quiz:
            with st.spinner("Extracting text..."):
                time.sleep(2)
                image = Image.open("image/example.png")
                st.sidebar.write("Original image")
                st.sidebar.image(image)
...
with tab2:
    ...
    st.header("Previous Quiz Score vs Projected Score for Next Quiz :sparkles:")
